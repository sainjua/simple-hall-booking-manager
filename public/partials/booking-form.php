...
<div class="shb-booking-form-wrapper">
				...
				<select name="hall_id" id="shb_hall_id" required>
					...
					<?php foreach ( $halls as $hall ) : ?>
						<option value="<?php echo esc_attr( $hall->id ); ?>" <?php selected( $selected_hall, $hall->id ); ?>>
							<?php echo esc_html( $hall->title ); ?> (<?php esc_html_e( 'Capacity:', 'simple-hall-booking-manager' ); ?> <?php echo esc_html( $hall->capacity ); ?>)
						</option>
					<?php endforeach; ?>
				</select>
			<?php endif; ?>
	...
	<div id="shb-date-selection-container" class="shb-form-group">
		...
		<p class="description"><?php esc_html_e( 'Click on dates to select. Select one date for single-day booking or multiple dates for multi-day booking.', 'simple-hall-booking-manager' ); ?></p>
		...
		<div id="shb-selected-dates-display" class="shb-selected-dates-display" style="display: none;">
			...
			<p class="shb-dates-count"><strong><?php esc_html_e( 'Total:', 'simple-hall-booking-manager' ); ?></strong> <span id="shb-dates-count">0</span> <?php esc_html_e( 'day(s)', 'simple-hall-booking-manager' ); ?></p>
		</div>
	</div>
		...
		<div class="shb-form-group">
			...
			<input type="text" name="event_purpose" id="shb_event_purpose" placeholder="<?php esc_attr_e( 'e.g., Birthday Party, Meeting, Wedding', 'simple-hall-booking-manager' ); ?>">
		</div>
