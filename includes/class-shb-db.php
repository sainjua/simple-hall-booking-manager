...
class SHB_DB
{
	...
	private function migrate_to_v110()
	{
		...
		$column_exists = $this->wpdb->get_results(
			$this->wpdb->prepare($sql, 'booking_type') // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		);
	...
	private function migrate_to_v120()
	{
		...
		// phpcs:ignore PluginCheck.Security.DirectDB.UnescapedDBParameter
		$column_exists = $this->wpdb->get_results(
			$this->wpdb->prepare($sql, 'pin') // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		);
	...
	private function migrate_to_v130()
	{
		...
		$has_booking_date_column = $this->wpdb->get_results(
			$this->wpdb->prepare($sql, 'booking_date') // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		);
		...
		if (!empty($has_booking_date_column)) {
			...
			foreach ($single_bookings as $booking) {
				...
				$has_dates = $this->wpdb->get_var(
					$this->wpdb->prepare($sql, $booking->id) // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
				);
		...
		// phpcs:ignore PluginCheck.Security.DirectDB.UnescapedDBParameter
		$column_exists = $this->wpdb->get_results(
			$this->wpdb->prepare($sql, 'slot_id') // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		);
	...
	private function migrate_to_v140()
	{
		...
		// phpcs:ignore PluginCheck.Security.DirectDB.UnescapedDBParameter
		$column_exists = $this->wpdb->get_results(
			$this->wpdb->prepare($sql, 'customer_organization') // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		);
	...
	public function get_hall($id)
	{
		...
		return $this->wpdb->get_row(
			$this->wpdb->prepare($sql, absint($id)) // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		);
	}
	...
	public function get_booking_by_token($token)
	{
		...
		// phpcs:ignore PluginCheck.Security.DirectDB.UnescapedDBParameter
		return $this->wpdb->get_row(
			$this->wpdb->prepare($sql, sanitize_text_field($token)) // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		);
	}
	...
	public function get_booking_dates($booking_id)
	{
		...
		$results = $this->wpdb->get_results(
			$this->wpdb->prepare($sql, $booking_id) // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		);
	...
	public function hall_has_full_day_slot($hall_id, $exclude_slot_id = 0)
	{
		...
		// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.PreparedSQL.InterpolatedNotPrepared, PluginCheck.Security.DirectDB.UnescapedDBParameter -- Table name interpolated above, params prepared
	...
	public function get_booking_by_pin($pin)
	{
		...
		// phpcs:ignore PluginCheck.Security.DirectDB.UnescapedDBParameter
		return $this->wpdb->get_row(
			$this->wpdb->prepare($sql, strtoupper(sanitize_text_field($pin))) // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		);
	}
}
