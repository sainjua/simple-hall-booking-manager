...
class SHB_AJAX {
	...
	private function verify_recaptcha( $token ) {
		...
		if ( is_wp_error( $response ) ) {
			error_log( 'SHB reCAPTCHA Error: ' . $response->get_error_message() );
		...
		if ( ! isset( $result->success ) || ! $result->success ) {
			error_log( 'SHB reCAPTCHA: Verification failed - ' . print_r( $result, true ) );
			return false;
		}

		// Check score (v3 only)
		if ( isset( $result->score ) && $result->score < $recaptcha_threshold ) {
			error_log( 'SHB reCAPTCHA: Score too low - ' . $result->score . ' (threshold: ' . $recaptcha_threshold . ')' );
			return false;
		}

		// Check action (optional but recommended)
		if ( isset( $result->action ) && 'booking_submit' !== $result->action ) {
			error_log( 'SHB reCAPTCHA: Invalid action - ' . $result->action );
			return false;
		}
	...
	public function submit_booking() {
		...
		if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
			error_log( 'SHB: Booking submission received' );
			error_log( 'SHB POST data: ' . print_r( $_POST, true ) );
		}

		// Verify nonce
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'shb_frontend_nonce' ) ) {
			if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
				error_log( 'SHB Error: Nonce verification failed' );
			}
			wp_send_json_error(
				array(
		...
		if ( ! $this->verify_recaptcha( $recaptcha_token ) ) {
			if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
				error_log( 'SHB Error: reCAPTCHA verification failed' );
			}
			wp_send_json_error(
				array(
		...
		$event_purpose   = isset( $_POST['event_purpose'] ) ? sanitize_text_field( $_POST['event_purpose'] ) : '';
		...
		if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
			error_log( 'SHB: Validated data - Hall: ' . $hall_id . ', Slot: ' . $slot_id . ', Date: ' . $booking_date . ', Multiday: ' . ( $is_multiday ? 'yes' : 'no' ) );
			if ( $is_multiday ) {
				error_log( 'SHB: Booking dates: ' . implode( ', ', $booking_dates ) );
			}
		}
		...
		if ( ! $hall_id || ! $slot_required || ! $date_required || ! $customer_name || ! $customer_email ) {
			...
			if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
				error_log( 'SHB Error: Missing fields - ' . implode( ', ', $missing_fields ) );
			}
		...
		if ( $is_multiday ) {
			...
			foreach ( $date_slots as $date => $s_id ) {
				...
				if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
					error_log( 'SHB: Validating slot for date ' . $date . ' - Slot ID: ' . $s_id );
				}
				
				if ( ! $slot ) {
					if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
						error_log( 'SHB Error: Slot not found - Slot ID: ' . $s_id . ' for date: ' . $date );
					}
					wp_send_json_error(
						array(
							'message' => sprintf(
				...
				if ( absint( $slot->hall_id ) !== absint( $hall_id ) ) {
					if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
						error_log( 'SHB Error: Hall mismatch for date ' . $date . ' - Slot Hall ID: ' . $slot->hall_id . ', Selected Hall ID: ' . $hall_id );
					}
					wp_send_json_error(
						array(
				...
				if ( ! $slot->is_active ) {
					if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
						error_log( 'SHB Error: Slot is not active - Slot ID: ' . $s_id . ' for date: ' . $date );
					}
					wp_send_json_error(
						array(
							'message' => sprintf(
			...
			if ( ! $slot ) {
				if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
					error_log( 'SHB Error: Slot not found - Slot ID: ' . $slot_id );
				}
				wp_send_json_error(
					array(
			...
			if ( ! $slot->is_active ) {
				if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
					error_log( 'SHB Error: Slot is not active - Slot ID: ' . $slot_id );
				}
				wp_send_json_error(
					array(
